...
| family | examples | worked instance (Data Client streaming SSR + live data) |
| --- | --- | --- |
...
**1. Context.** Read the project's goals document (often `GOALS.md`; it ranks which costs matter). Separate earlier documents into findings (real runs) and ideas (proposals). List every host/flavor the invariants must hold on.
...
Read [Document template](references/template.md) when writing the document: skeleton, critical-variables table, storyboard legend, worked snippets, and the minimum measurement recipe. Keep the document in the project's planning store or `docs/` as directed; it is user-facing.
